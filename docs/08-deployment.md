...
Log level is configurable via `--log-level` flag or `DOCKESTRA_LOG_LEVEL` env var
...
services:
  master:
    ...
    environment:
      ...
      DOCKESTRA_TLS_CERT: /tls/server.crt   # optional
      DOCKESTRA_TLS_KEY: /tls/server.key    # optional
...
```
DOCKESTRA_MASTER_KEY=<hex-key>
DOCKESTRA_DB_PATH=/var/lib/orkestra/orkestra.db
DOCKESTRA_AGENT_ADDR=0.0.0.0:8443
DOCKESTRA_UI_ADDR=0.0.0.0:8080
```
...
```
DOCKESTRA_MASTER_ADDR=https://master.example.com:8443
DOCKESTRA_AGENT_DATA=/etc/orkestra/agent
DOCKESTRA_LOG_LEVEL=info
```
...
**Docker images:**
- `ghcr.io/egt/orkestra-master:{version}`
- Multi-arch manifest (amd64 + arm64)

**CI (GitLab, egt cicd-components):**

```yaml
# .gitlab-ci.yml
include:
  - project: egt/cicd-components
    ref: main
...
Pipeline stages: `next-version` → `build+test+lint` → `release` (tag only) → `deploy-master` (optional).
...
|---|---|---|
| SQLite database | `DOCKESTRA_DB_PATH` | Daily (at minimum) |
| Master key | `DOCKESTRA_MASTER_KEY` | On creation, stored in password manager / HSM |
...
3. Set `DOCKESTRA_MASTER_KEY` to the same value as before.
